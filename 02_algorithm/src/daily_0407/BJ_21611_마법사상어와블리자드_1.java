...
public class BJ_21611_마법사상어와블리자드_1 {
	...
	public static void main(String[] args) throws Exception{
		...
		shark=new Point((n+1)/2-1, (n+1)/2-1,4); //상어 좌표는 고정. 빈칸이 아님을 나타내기 위해 숫자 4로 설정
		...
		for(int i=0;i<m;i++) { //블리자드 m 번 시전
			...
			for(int round=0;round<n-1;round++) {
				...
				for(int j=0;j<round+1;j++) { //좌, 우
					if(map[x][y].num==0) {
						vacant=new Point(x,y,0); //빈칸의 좌표 저장해둠
					}
					now=map[x][y].num;
					dx=x+snail[d][0];
					dy=y+snail[d][1];
					System.out.println("dx,dy: "+dx+","+dy);
					if(vacant !=null && map[dx][dy].num!=0) {
						map[vacant.x][vacant.y].num=map[dx][dy].num;
						vacant=null;
						print(map);
